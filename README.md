...
本仓库当前阶段为**基础功能已实现**：设计文档、数据库表结构、基础设施（统一响应 / 异常 / JWT 认证骨架 / 可插拔 MQ 抽象 / 本地消息表）均已落地，业务侧实现了**注册登录、发帖、评论、点赞**四条链路，并配套 Vue 3 前端。收藏、关注、通知、搜索、后台管理与秒杀模块尚未开始。
...
| 顺序 | 文档 | 适合谁 |
|---|---|---|
| 1 | [需求分析](docs/01-需求分析.md) | 想了解做什么、为什么做、做到什么程度 |
| 2 | [架构设计](docs/02-架构设计.md) | 想了解整体结构、技术选型、关键决策（ADR） |
| 3 | [功能设计](docs/03-功能设计.md) | 想了解每个功能怎么实现、接口长什么样 |
| 4 | [数据库设计](docs/04-数据库设计.md) | 想了解表结构、索引、缓存 Key 的设计理由 |
| 5 | [MQ 选型对比方案](docs/05-消息中间件选型对比方案.md) | 想了解 Kafka vs RabbitMQ 怎么测、怎么选 |
...
| 约束 | 说明 |
|---|---|
| 秒杀每人限购 1 张 | 若需放开，`t_seckill_order` 的唯一索引必须调整（详见数据库设计 4.6 节） |
| 评论仅两级 | 不支持无限层级回复，这是有意为之（详见数据库设计 4.3 节） |
| 不使用统一逻辑删除列 | 由 `status` 字段承担状态语义（详见数据库设计 1.3 节） |
| 单表 2000 万行上限 | 超过后触发分表，`t_user_like` 分片键已选定为 `target_id` |
| 通知为轮询非推送 | 30 秒延迟，不引入 WebSocket（详见功能设计 5.3 节） |
...
| 阶段 | 内容 | 触发条件 |
|---|---|---|
| V1.0（当前） | 基础闭环完成：注册登录 / 发帖 / 评论 / 点赞 + 前端 | — |
| V1.1 | 可观测性补齐（Prometheus + Grafana + Zipkin） | V1.0 上线 |
| V1.2 | MQ 压测对比结论落地，确定主用中间件 | 压测完成 |
| V2.0 | `t_user_like` 分表（16 片） | 单表 > 2000 万行 |
| V3.0 | 按模块边界拆分微服务 | 日活 > 5 万 |
